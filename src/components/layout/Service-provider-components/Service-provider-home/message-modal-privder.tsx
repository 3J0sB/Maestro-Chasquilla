...
const MessageModal: React.FC<MessageModalProps> = ({
  ...
  const handleSendMessage = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const conversationResponse = await fetch('/api/service-provider/conversations', {
        method: 'POST',
        headers: {
      ...
      const messageResponse = await fetch('/api/service-provider/conversations/messages', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
      ...
      // Opcional: redirigir al proveedor a la página de mensajes
      router.push('/service-provider/messages');
    } catch (error) {
      ...
      <div className="bg-white rounded-lg shadow-xl w-full max-w-md overflow-hidden">
        ...
        <div className="px-4 py-3 border-b border-gray-200 bg-gray-50">
          <div className="flex items-center">
            ...
            <div>
              <p className="font-medium text-gray-800">Para: {userName}</p>
              <p className="text-xs text-gray-500">Solicitud: {serviceType}</p>
            </div>
          </div>
        </div>
        ...
        <form onSubmit={handleSendMessage}>
          <div className="p-4">
            ...
            <div className="text-xs text-gray-500 mt-1">
              Puedes proporcionar más detalles sobre el servicio o coordinar una visita.
            </div>
          </div>
